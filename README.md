...
0.2.1 的补丁还修正了辅助新标签的引导状态判断：等官方状态查询成功后再决定是否展示引导，避免把尚未开始的查询误判为“未完成”。真正首次使用仍保留引导；读取失败交由官方错误界面处理，不会自动接受引导或改写完成标志。此修复也需要手动应用补丁，仅升级 VSIX 不会修改官方文件。
...
已应用 0.2.0 标题补丁的用户，更新源码后先执行 `title:restore`，再执行 `title:apply`，以应用新的组合补丁。请先结束运行中的任务并保存草稿，再重载窗口。
...
code --install-extension codex-multi-tab-0.2.1.vsix
```

...
原生界面与侧栏数据能力依赖官方扩展的内部接口及少量资产补丁。扩展会校验已审计文件、保留原始备份，并在启动或官方扩展变化时检查兼容性；**不保证所有后续 Codex 更新自动兼容**。核心桥接和原生历史增强分开检查，某组资产变化时会只停用该组，不会为了原生历史增强而阻断官方功能和独立标签入口。

- 当前 **0.5.5** 回归覆盖官方 **26.917.62051（macOS ARM64）** 的核心桥接资产副本，以及 **26.917.61114** 的完整增强资产副本。62051 的侧栏桥、独立标签标题和首次引导已按精确哈希验证；其原生历史增强暂未启用。Windows、Linux、macOS Intel 尚未验证。
- 版本号变化但核心桥接资产字节完全相同时，可复用兼容配置；核心桥接哈希或内部结构变化时拒绝修改并提示需要适配。仅原生历史资产变化时进入 core-only 降级。
- 基础新建标签也依赖官方内部 URI，其变化可能需要适配。核心桥接不可用时侧栏显示兼容提示，仍保留新建标签入口；核心桥接可用但原生历史未审计时，会显示降级状态并保留账号、用量和项目会话等基础能力。
